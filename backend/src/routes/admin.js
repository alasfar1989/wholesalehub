...
router.get('/dashboard', async (req, res) => {
  try {
    const [users, listings, featured, ratings, pendingRatings, escrows] = await Promise.all([
      ...
      db.query(`SELECT
        COUNT(*) as total,
        COUNT(*) FILTER (WHERE status IN ('pending_seller','deposit_pending','pending_payment','payment_received','shipped_to_warehouse','at_warehouse','shipped','delivered')) as active,
